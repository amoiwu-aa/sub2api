...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = `# Codex CLI → RingStar Grok group
...
# Grok/RingStar path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at RingStar OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via RingStar'
