    ...
    <template v-if="isAdmin">
      ...
      <transition name="dropdown">
        <div
          v-if="dropdownOpen"
          ref="dropdownRef"
          class="absolute left-0 z-50 mt-2 w-64 overflow-hidden whitespace-normal rounded-xl border border-gray-200 bg-white shadow-lg transition-all duration-200 dark:border-dark-700 dark:bg-dark-800"
        >
          ...
          <div class="p-4">
            ...
            <template v-else>
              ...
              <!-- Notify only: in-place update/rollback is disabled for this deployment -->
              <div v-if="hasUpdate" class="space-y-2">
                <component
                  :is="changelogUrl ? 'a' : 'div'"
                  v-bind="
                    changelogUrl
                      ? { href: changelogUrl, target: '_blank', rel: 'noopener noreferrer' }
                      : {}
                  "
                  class="flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-800/50 dark:bg-amber-900/20"
                  :class="
                    changelogUrl
                      ? 'group transition-colors hover:bg-amber-100 dark:hover:bg-amber-900/30'
                      : ''
                  "
                >
                  <div
                ...
                >
                  <svg
                    class="mt-px h-3.5 w-3.5 flex-shrink-0 text-blue-500 dark:text-blue-400"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2"
                  ...
                  <p class="text-xs leading-4 text-blue-600 dark:text-blue-400">
                    {{ t('version.inPlaceUpdateDisabledHint') }}
                  </p>
                </div>
              </div>
