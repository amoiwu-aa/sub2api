<template>
  ...
  >
    ...
    >
      ...
      >
        <div
          v-if="isCursorSandAccount"
          class="mb-4 rounded-lg border border-teal-200 bg-teal-50 p-3 dark:border-teal-800/60 dark:bg-teal-900/20"
        ...
        >
          ...
          <div
            class="mt-4 border-t border-teal-200 pt-3 dark:border-teal-800/60"
            data-testid="cursor-sand-model-whitelist"
          >
            <label class="input-label text-teal-900 dark:text-teal-200">{{
            ...
            <p class="text-xs text-teal-700 dark:text-teal-400">
              ...
              <span v-if="allowedModels.length === 0">{{
                t('admin.accounts.cursor.sandSupportsAllModels')
              }}</span>
            </p>
          </div>
        </div>
        <template v-if="!isCursorSandAccount">
          <div class="flex items-center justify-between gap-4">
            <div>
...
const isCursorSandCredentials = (credentials?: Record<string, unknown>) => {
  const raw = credentials?.cursor_agent_profile ?? credentials?.agent_profile
  if (typeof raw !== 'string') return false
  return ['sand', 'grok-bot', 'grok_bot'].includes(raw.trim().toLowerCase())
}
const isCursorSandAccount = computed(() => {
  if (props.account?.platform !== 'cursor') return false
  const credentials = (props.account.credentials as Record<string, unknown> | undefined) || {}
  return isCursorSandCredentials(credentials)
})
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  cursorForceUseEnabled.value =
    newAccount.platform === 'cursor' &&
    !isCursorSandAccount.value &&
    extra?.cursor_force_use === true
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    // Load model mappings for OAuth accounts that expose account-level model restrictions.
    const oauthCredentials = newAccount.credentials as Record<string, unknown> | undefined
    const supportsOAuthModelRestriction =
      newAccount.platform === 'openai' ||
      newAccount.platform === 'grok' ||
      (newAccount.platform === 'cursor' && isCursorSandCredentials(oauthCredentials))
    if (supportsOAuthModelRestriction && oauthCredentials) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Persist OAuth model restrictions, including the Grok Bot/Sand whitelist.
    const supportsOAuthModelRestriction =
      props.account.platform === 'openai' ||
      props.account.platform === 'grok' ||
      (props.account.platform === 'cursor' && isCursorSandAccount.value)
    if (supportsOAuthModelRestriction && props.account.type === 'oauth') {
    ...
    if (props.account.platform === 'cursor') {
      const currentExtra =
        ...
        {}
      const newExtra: Record<string, unknown> = { ...currentExtra }
      if (isCursorSandAccount.value) {
        delete newExtra.cursor_force_use
      } else if (cursorForceUseEnabled.value) {
