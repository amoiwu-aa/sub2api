...
describe('AccountUsageCell', () => {
  ...
  it('Sand 账号只展示独立的 Grok Bot 周额度', async () => {
    const wrapper = mountCursor({
      cursor_plan: 'ultra',
      cursor_sand_usage: {
        utilization: 0.024486,
        resets_at: '2026-08-27T21:58:49.552Z',
        remaining_seconds: 600000
      },
      cursor_sand_has_available_usage: true,
      cursor_sand_available_banked_resets: 2,
      cursor_auto_usage: { utilization: 64.2545, resets_at: '2026-09-01T05:44:02Z' },
      cursor_api_usage: { utilization: 15.006, resets_at: '2026-09-01T05:44:02Z' },
      cursor_included_used: 400,
      cursor_included_limit: 400
    }, 3005, {
      credentials: { cursor_agent_profile: 'sand' }
    })

    await flushPromises()

    expect(wrapper.text()).toContain('admin.accounts.cursor.sandQuotaBadge')
    expect(wrapper.text()).toContain('admin.accounts.cursor.sandUsageLabel|0.024486')
    expect(wrapper.text()).toContain('admin.accounts.cursor.sandBankedResets')
    expect(wrapper.text()).not.toContain('admin.accounts.cursor.autoLabel|64.2545')
    expect(wrapper.text()).not.toContain('admin.accounts.cursor.apiLabel|15.006')
    expect(wrapper.text()).not.toContain('$400.00 / $400.00')
    expect(wrapper.text()).not.toContain('admin.accounts.cursor.plan.ultra')
  })
