export default {
    accounts: {
      ...
      dataExportCsvNotice: '清单只列出凭据的字段名，不含任何密钥值；用于盘点与交接，不能再导入回来。',
      dataExportCredentialsNotice: '导出各账号在本机上的原生凭证原文（等同密码），请妥善保存。',
      dataExportFormat: '导出格式',
      dataExportFormatBackup: '备份 JSON',
      dataExportFormatBackupDesc: '完整备份，含凭据原文，可从「导入」原样还原。',
      dataExportFormatCsv: 'CSV 清单',
      dataExportFormatCsvDesc: '账号清单表格，不含密钥值，可直接用 Excel 打开。',
      dataExportFormatCredentials: '原生凭证',
      dataExportFormatCredentialsDesc: 'Kiro 导出 kiro-auth-token.json、Cursor 导出 Cookie 原文，可粘回「添加账号」。',
      dataExportEmpty: '没有可导出的账号',
      ...
      moreFilters: '更多筛选',
      clearFilters: '清除筛选',
      privacyFilterLabel: 'Privacy 状态',
      groupFilterLabel: '分组',
      mobileShowDetails: '展开详情',
      mobileHideDetails: '收起详情',
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。RingStar 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
        ...
        failed: '失败',
        notProbed: '未探测',
        modelScoped: '按模型 · {count}',
        modelPricingTitle: 'New API 模型倍率声明',
        modelCounts: '倍率计费：{ratio} 个；固定价格：{fixed} 个',
        modelRatioRange: '模型倍率范围：{value}',
        modelPriceRange: '固定价格范围：{value}',
        groupRatios: '站点分组倍率：{value}',
        modelExamples: '代表模型',
        ratioModelLine: '{model}：输入 {input}x / 输出 {output}x',
        fixedModelLine: '{model}：固定价 {price}',
        sourceEndpoint: '声明来源：{value}',
        modelScopeNoSync: '按模型声明不会自动覆盖账号统一倍率',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按 RingStar 近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      proxyTrafficToday: '当日 {value}',
      proxyTrafficTotal: '累计 {value}',
      proxyTrafficHint: '代理 HTTP 请求与响应体流量；自启用统计后开始累计。',
      ...
      kiro: {
        ...
        pasteFileSub: '本机 token 文件',
        pasteHintShort: '需要本机装有 Kiro 并已登录。',
        webLogin: '网页登录',
        pasteFile: '粘贴本机文件',
        webHint: '推荐。服务器生成登录链接，你在浏览器里登录即可，不需要本机装 Kiro。支持 Google / GitHub 与企业版（IAM Identity Center）。',
        generateLoginUrl: '生成登录链接',
        loginUrlLabel: '登录链接',
        idcLoginUrlLabel: '第二段登录链接（AWS）',
        idcRequiredHint: '这是企业账号（{provider}）。Kiro 那一步只是把你转交给 AWS，还需要再登录一次：打开下面的链接完成授权，然后把新的回调地址粘回来。',
      ...
      cursor: {
        ...
        ideProfileSub: '使用 agent.api5.cursor.sh 的 IDE 会话协议',
        sandProfile: 'Grok Bot / Sand',
        sandProfileSub: '使用 api2.cursor.sh 的桌面客户端协议',
        sandQuotaBadge: 'Grok Bot / Sand',
        sandQuotaHint: 'Grok Bot 独立周额度，仅统计通过 Sand 通道发出的模型请求。',
        sandModelRestrictionTitle: 'Grok Bot 模型限制',
        sandModelRestrictionHint: '仅会调度下方选择的 Grok Bot/Sand 模型，并始终使用 api2.cursor.sh；不会回退到 Cursor IDE，也不会读取或消耗 Cursor Models、Other Models 额度。点击“同步上游模型”可读取此账号当前真实可用模型。',
        sandSupportsAllModels: '未选择时允许该账号上游返回的全部已确认 Grok Bot 模型',
        sandUsageLabel: '周额',
        sandPooled: '团队共享',
        sandExhausted: '周额度耗尽',
        sandBankedResets: '可用额度重置次数：{count}',
        sandImportHint: '粘贴 Grok Bot 运行时的已解密访问令牌。sand-secrets.json 中的加密包装值不能直接使用。',
        sandAccessTokenLabel: 'Sand 访问令牌',
        sandAccessTokenPlaceholder: '粘贴运行时 access token',
        sandRefreshTokenLabel: 'Sand 刷新令牌',
        machineIdLabel: '机器 ID',
        sandNamespaceLabel: '沙盒命名空间',
        sandImportSecurityHint: '不要粘贴 scoped:v1 或 plaintext:v1 开头的磁盘加密值；可改用浏览器登录。',
        modelOptions: 'Cursor 模型参数',
        modelOptionsHint: 'Effort、Fast 与 MAX 可独立组合；未生成额外的重复模型名称。',
        effort: '推理强度',
        effortLow: '低（Low）',
        effortMedium: '中（Medium）',
        effortHigh: '高（High）',
        effortXHigh: '超高（Extra High）',
        fast: 'Fast 模式',
        maxMode: 'MAX 模式',
        forceUse: '强制使用 Cursor 账号',
        forceUseDesc: '忽略 RingStar 根据 Cursor 额度快照产生的本地额度拦截，继续向上游发起请求。',
        forceUseWarning:
          '仅绕过本地额度 429 和额度耗尽停调，不会绕过 Cursor 上游的真实限流、并发或鉴权错误；额度用尽后可能产生 On-Demand 费用。',
        browserLoginSub: '浏览器登录并自动轮询',
        pasteCookieSub: '已有的 Session Cookie',
        apiToken: '接口令牌',
        apiTokenSub: 'Grok Bot / Sand Token',
        apiTokenHint: '填入 Grok Bot / Sand 运行时访问令牌，服务端会按桌面客户端接口协议连接上游。',
        apiTokenLabel: '接口访问令牌',
        apiTokenPlaceholder: '粘贴 Grok Bot / Sand access token',
        browserLogin: '浏览器登录',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 RingStar 实例的场景。',
      ...
      expired: '已过期',
      expiresWithin24Hours: '24 小时内到期',
      expiresWithin3Days: '3 天内到期',
      chatgptCookieSource: 'Cookie 导入',
      proxy: '代理',
      noProxy: '无代理',
      concurrency: '并发数',
      loadFactor: '负载因子',
      ...
      oauth: {
        ...
        openai: {
          ...
          chatgptCookieAuth: 'ChatGPT Cookie 导入',
          chatgptCookieDesc: '粘贴 ChatGPT 浏览器登录态，系统会通过官方会话接口换取 Access Token 并创建 OpenAI 账号。',
          chatgptCookieInputLabel: 'ChatGPT Cookie',
          chatgptCookiePlaceholder: '支持 Cookie-Editor JSON、Cookie 请求头或 Netscape 格式\n\n示例：\n__Secure-next-auth.session-token=...',
          chatgptCookieHint: '可直接粘贴 Cookie-Editor 导出的 JSON；系统会自动识别格式和分片会话 Cookie。',
          chatgptCookieSecurity: '浏览器 Cookie 属于高敏感凭证。仅在可信设备上使用；服务端只会将其发送到官方 ChatGPT HTTPS 会话接口，不会保存为账号凭证或写入审计日志。',
          chatgptCookieAdvanced: '高级选项（可选）',
          chatgptCookieUserAgentLabel: '导出 Cookie 时使用的 User-Agent',
          chatgptCookieUserAgentPlaceholder: '留空则使用服务端默认浏览器标识',
          chatgptCookieUserAgentHint: '仅当会话校验与浏览器标识绑定时填写，建议与导出 Cookie 的浏览器保持一致。',
          chatgptCookieConverting: '正在转换并创建...',
          chatgptCookieImportAndCreate: '转换 Cookie 并创建账号',
          chatgptCookieEmpty: '请粘贴 ChatGPT Cookie',
          chatgptCookieImportFailed: 'ChatGPT Cookie 转换或账号导入失败',
          chatgptCookiePreviewButton: '仅验证',
          chatgptCookiePreviewing: '正在安全验证...',
          chatgptCookiePreviewFailed: 'ChatGPT Cookie 验证失败',
          chatgptCookiePreviewSuccess: '验证成功，未返回 Cookie 或 Access Token',
          chatgptCookiePreviewFormat: '识别格式',
          chatgptCookiePreviewCount: '有效 Cookie 数量',
          chatgptCookiePreviewHost: '官方目标域名',
          chatgptCookiePreviewExpiry: 'Access Token 到期时间',
          chatgptCookiePreviewEmail: '账号邮箱',
          chatgptCookiePreviewAccount: 'ChatGPT 账号 ID',
          chatgptCookiePreviewPlan: '套餐',
          chatgptCookiePreviewSafe: '修改 Cookie 或 User-Agent 后，预检结果会自动清除。',
          chatgptCookieReimport: '更新此账号凭证',
          chatgptCookieReimportSuccess: 'ChatGPT Cookie 已重新导入，账号凭证已更新',
          chatgptCookieReimportFailed: 'Cookie 与所选账号不匹配或重新导入失败',
