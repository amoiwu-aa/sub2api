export default {
    accounts: {
      ...
      dataExportCsvNotice: 'The inventory lists credential field names only — no secret values. Useful for audits and handovers; it cannot be imported back.',
      dataExportCredentialsNotice: 'Exports each account\'s raw on-disk credential (equivalent to a password). Store it securely.',
      dataExportFormat: 'Export format',
      dataExportFormatBackup: 'Backup JSON',
      dataExportFormatBackupDesc: 'Full backup with raw credentials; restores as-is via Import.',
      dataExportFormatCsv: 'CSV inventory',
      dataExportFormatCsvDesc: 'Account spreadsheet without secret values; opens directly in Excel.',
      dataExportFormatCredentials: 'Native credentials',
      dataExportFormatCredentialsDesc: 'Kiro exports kiro-auth-token.json, Cursor exports the raw cookie — paste them back into Create Account.',
      dataExportEmpty: 'No accounts to export',
      ...
      privacyFilterLabel: 'Privacy',
      groupFilterLabel: 'Group',
      mobileShowDetails: 'Show details',
      mobileHideDetails: 'Hide details',
      ...
      upstreamBilling: {
        trustWarning: 'This rate is declared by the upstream site for the current API key. RingStar cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.',
        ...
        modelScoped: 'Per model · {count}',
        modelPricingTitle: 'New API model rate declaration',
        modelCounts: 'Ratio-priced: {ratio}; fixed-price: {fixed}',
        modelRatioRange: 'Model ratio range: {value}',
        modelPriceRange: 'Fixed price range: {value}',
        groupRatios: 'Site group ratios: {value}',
        modelExamples: 'Representative models',
        ratioModelLine: '{model}: input {input}x / output {output}x',
        fixedModelLine: '{model}: fixed price {price}',
        sourceEndpoint: 'Declaration source: {value}',
        modelScopeNoSync: 'Per-model declarations do not overwrite the account-wide rate',
      ...
      proxyTrafficToday: 'Today {value}',
      proxyTrafficTotal: 'Total {value}',
      proxyTrafficHint: 'Proxy HTTP request and response-body traffic, accumulated after tracking is enabled.',
      ...
      kiro: {
        ...
        pasteFileSub: 'Local token file',
        pasteHintShort: 'Requires Kiro installed and signed in locally.',
        webLogin: 'Web login',
        pasteFile: 'Paste local file',
        webHint: 'Recommended. The server generates a login link and you sign in from your browser — no local Kiro install needed. Works with Google / GitHub and enterprise (IAM Identity Center) accounts.',
        generateLoginUrl: 'Generate login link',
        loginUrlLabel: 'Login link',
        idcLoginUrlLabel: 'Second login link (AWS)',
        idcRequiredHint: 'This is an enterprise account ({provider}). The Kiro step only handed you off to AWS, so one more sign-in is needed: open the link below, authorize, then paste the new callback address back here.',
      ...
      cursor: {
        clientProfile: 'Client protocol',
        ideProfile: 'Cursor IDE',
        ideProfileSub: 'Use the agent.api5.cursor.sh IDE session protocol',
        sandProfile: 'Grok Bot / Sand',
        sandProfileSub: 'Use the api2.cursor.sh desktop-client protocol',
        sandQuotaBadge: 'Grok Bot / Sand',
        sandQuotaHint: 'Independent Grok Bot weekly usage for requests sent through the Sand transport.',
        sandModelRestrictionTitle: 'Grok Bot model restriction',
        sandModelRestrictionHint: 'Only the Grok Bot/Sand models selected below are scheduled, always through api2.cursor.sh. This account never falls back to Cursor IDE or reads Cursor Models / Other Models quota. Use Sync upstream models to load the models currently available to this account.',
        sandSupportsAllModels: 'When empty, allow every verified Grok Bot model returned for this account',
        sandUsageLabel: 'Week',
        sandPooled: 'Team pooled',
        sandExhausted: 'Weekly quota exhausted',
        sandBankedResets: 'Available quota resets: {count}',
        sandImportHint: 'Paste a decrypted runtime token from Grok Bot. Values wrapped by Electron safeStorage in sand-secrets.json cannot be used directly.',
        sandAccessTokenLabel: 'Sand access token',
        sandAccessTokenPlaceholder: 'Paste the runtime access token',
        sandRefreshTokenLabel: 'Sand refresh token',
        machineIdLabel: 'Machine ID',
        sandNamespaceLabel: 'Sandbox namespace',
        sandImportSecurityHint: 'Do not paste disk values beginning with scoped:v1 or plaintext:v1. Use browser login instead.',
        modelOptions: 'Cursor model options',
        modelOptionsHint: 'Combine Effort, Fast, and MAX independently without duplicate model names.',
        effort: 'Effort',
        effortLow: 'Low',
        effortMedium: 'Medium',
        effortHigh: 'High',
        ...
        fast: 'Fast mode',
        maxMode: 'MAX mode',
        forceUse: 'Force use Cursor account',
        forceUseDesc: 'Ignore RingStar quota blocks derived from Cursor usage snapshots and continue sending requests upstream.',
        forceUseWarning:
          'This only bypasses local quota 429s and quota-exhaustion parking. It cannot bypass real Cursor rate, concurrency, or authentication errors, and may incur on-demand charges.',
        browserLoginSub: 'Browser sign-in, polled automatically',
        pasteCookieSub: 'An existing session cookie',
        apiToken: 'API token',
        apiTokenSub: 'Grok Bot / Sand token',
        apiTokenHint: 'Paste the Grok Bot / Sand runtime token. The server will use the desktop-client protocol for the upstream connection.',
        apiTokenLabel: 'API access token',
        apiTokenPlaceholder: 'Paste the Grok Bot / Sand access token',
      ...
      poolModeInfo:
        'When enabled, upstream 429/403/401 errors will auto-retry without marking the account as rate-limited or errored. Suitable for upstream pointing to another RingStar instance.',
      ...
      expired: 'Expired',
      expiresWithin24Hours: 'Expires within 24 hours',
      expiresWithin3Days: 'Expires within 3 days',
      chatgptCookieSource: 'Cookie import',
      proxy: 'Proxy',
      noProxy: 'No Proxy',
      ...
      oauth: {
        ...
        openai: {
          ...
          chatgptCookieAuth: 'ChatGPT Cookie Import',
          chatgptCookieDesc: 'Paste a ChatGPT browser session. RingStar exchanges it through the official session endpoint and creates an OpenAI account from the access token.',
          chatgptCookieInputLabel: 'ChatGPT Cookie',
          chatgptCookiePlaceholder: 'Cookie-Editor JSON, a Cookie header, or Netscape format\n\nExample:\n__Secure-next-auth.session-token=...',
          chatgptCookieHint: 'You can paste Cookie-Editor JSON directly. Chunked session cookies and the input format are detected automatically.',
          chatgptCookieSecurity: 'Browser cookies are highly sensitive. Use this only on a trusted device. The server sends them only to the official ChatGPT HTTPS session endpoint and never stores them as account credentials or in audit logs.',
          chatgptCookieAdvanced: 'Advanced options (optional)',
          chatgptCookieUserAgentLabel: 'User-Agent used when exporting the cookie',
          chatgptCookieUserAgentPlaceholder: 'Leave blank to use the server default',
          chatgptCookieUserAgentHint: 'Set this only when session validation is browser-bound; use the User-Agent of the browser that exported the cookie.',
          chatgptCookieConverting: 'Converting and creating...',
          chatgptCookieImportAndCreate: 'Convert Cookie & Create Account',
          chatgptCookieEmpty: 'Please paste a ChatGPT cookie',
          chatgptCookieImportFailed: 'Failed to convert the ChatGPT cookie or import the account',
          chatgptCookiePreviewButton: 'Validate Only',
          chatgptCookiePreviewing: 'Validating securely...',
          chatgptCookiePreviewFailed: 'Failed to validate the ChatGPT cookie',
          chatgptCookiePreviewSuccess: 'Validation passed without returning the Cookie or access token',
          chatgptCookiePreviewFormat: 'Detected format',
          chatgptCookiePreviewCount: 'Applicable cookies',
          chatgptCookiePreviewHost: 'Official destination',
          chatgptCookiePreviewExpiry: 'Access token expires',
          chatgptCookiePreviewEmail: 'Account email',
          chatgptCookiePreviewAccount: 'ChatGPT account ID',
          chatgptCookiePreviewPlan: 'Plan',
          chatgptCookiePreviewSafe: 'Changing the Cookie or User-Agent automatically clears this preview.',
          chatgptCookieReimport: 'Update This Account',
          chatgptCookieReimportSuccess: 'ChatGPT Cookie re-imported and account credentials updated',
          chatgptCookieReimportFailed: 'The Cookie does not match the selected account or re-import failed',
