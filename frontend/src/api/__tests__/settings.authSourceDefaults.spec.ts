...
/** 全 null 的全平台 map（平台清单取自实现，避免测试单独维护一份而漂移） */
const allNullQuotas: DefaultPlatformQuotasMap = Object.fromEntries(
  PLATFORM_QUOTA_PLATFORMS.map((p) => [p, { daily: null, weekly: null, monthly: null }]),
...
describe("normalizePlatformQuotasMap", () => {
  ...
  it("无参数时返回全平台全 null", () => {
    ...
    expect(Object.keys(result)).toHaveLength(PLATFORM_QUOTA_PLATFORMS.length);
