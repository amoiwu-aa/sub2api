...
//
// /v1/usage、/v1/sub2api/billing、/v1/sub2api/usage 端点与异步生图任务查询只需鉴权，不需要计费执行。
...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		sessionUsageRequest := c.Request.URL.Path == "/v1/sub2api/usage"
		// Async image task polling only reads data that already belongs to the
		// authenticated key and must remain available after the completed
		// generation consumes the key's remaining balance.
		skipBilling := c.Request.URL.Path == "/v1/usage" || billingInfoRequest || sessionUsageRequest || isAsyncImageTaskRead(c.Request.Method, c.Request.URL.Path)
		...
		if cfg.RunMode == config.RunModeSimple {
			...
			if !billingInfoRequest && !sessionUsageRequest {
		...
		if isSubscriptionType && subscriptionService != nil && !billingInfoRequest && !sessionUsageRequest {
