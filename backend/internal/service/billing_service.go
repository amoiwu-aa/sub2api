...
type ModelPricing struct {
	...
	FastMultiplier                     *float64 // 渠道显式 Fast/priority 倍率
	FlexMultiplier                     *float64 // 渠道显式 Flex 倍率
...
func usePriorityServiceTierPricing(serviceTier string, pricing *ModelPricing) bool {
	...
	if tier != "priority" && tier != ServiceTierFast {
		return false
	}
	if pricing.FastMultiplier != nil || (tier == ServiceTierFast && useFastServiceTierPricing(serviceTier, pricing)) {
		return false
	}
...
func useFastServiceTierPricing(serviceTier string, pricing *ModelPricing) bool {
	if pricing == nil || pricing.FastMultiplier != nil || normalizeBillingServiceTier(serviceTier) != ServiceTierFast {
		return false
	}
