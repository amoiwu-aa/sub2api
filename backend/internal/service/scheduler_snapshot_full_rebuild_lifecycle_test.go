...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Len(t, cache.tokens(), 2*canonicalBucketCount, "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, 2*canonicalBucketCount)
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	require.Equal(t, 3*canonicalAccountQueryCount, accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.Equal(t, canonicalBucketCount, reopenCount)
		require.Equal(t, canonicalBucketCount+1, capturesAtFirstDB, "C(0) and the historical bucket must be captured before DB")
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
	...
	require.Equal(t, 2*canonicalAccountQueryCount+1, accounts.callCount())
	require.Equal(t, canonicalAccountQueryCount+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildOrdinaryCaptureErrorReturnsBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, canonicalBucketCount+2, cache.captureAttemptCount(), "all canonical and ordinary captures must be attempted before returning")
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	require.Equal(t, 2*canonicalBucketCount+3, cache.captureAttemptCount())
	require.Equal(t, 2*canonicalAccountQueryCount+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, canonicalAccountQueryCount, accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Zero(t, accounts.callCount())
	require.Zero(t, cache.totalSetAttempts())
	require.Equal(t, canonicalBucketCount+1, len(cache.retiredBuckets()))
	...
	require.Equal(t, 3*canonicalBucketCount+3, len(cache.retiredBuckets()))
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.False(t, held)
		require.Equal(t, canonicalBucketCount, reopenCount)
		require.Equal(t, canonicalBucketCount+7, capturesAtFirstDB)
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Zero(t, fallbackCalls)
	require.Empty(t, freshCalls)
	require.Equal(t, canonicalBucketCount+3, cache.captureAttemptCount())
	require.Equal(t, canonicalAccountQueryCount+3, accounts.callCount())
	require.Equal(t, canonicalAccountQueryCount+3, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, 2*canonicalAccountQueryCount, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 4*canonicalAccountQueryCount, accounts.callCount())
