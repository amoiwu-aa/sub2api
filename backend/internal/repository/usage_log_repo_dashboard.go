...
func (r *usageLogRepository) fillDashboardUsageStatsAggregated(ctx context.Context, stats *DashboardStats, todayUTC, now time.Time) error {
	totalStatsQuery := `
		SELECT
			...
			COALESCE(SUM(reported_input_tokens), 0) as total_reported_input_tokens,
			COALESCE(SUM(reported_cache_creation_tokens), 0) as total_reported_cache_creation_tokens,
			COALESCE(SUM(reported_forced_cache_read_tokens), 0) as total_reported_forced_cache_read_tokens,
	...
	if err := scanSingleRow(
		...
		&stats.TotalReportedInputTokens,
		&stats.TotalReportedCacheCreationTokens,
		&stats.TotalReportedForcedCacheReadTokens,
	...
	todayStatsQuery := `
		SELECT
			...
			reported_input_tokens as today_reported_input_tokens,
			reported_cache_creation_tokens as today_reported_cache_creation_tokens,
			reported_forced_cache_read_tokens as today_reported_forced_cache_read_tokens,
	...
	if err := scanSingleRow(
		...
		&stats.TodayReportedInputTokens,
		&stats.TodayReportedCacheCreationTokens,
		&stats.TodayReportedForcedCacheReadTokens,
	...
	combinedStatsQuery := `
		...
		SELECT
			...
			COALESCE(SUM(input_tokens)
				FILTER (WHERE created_at >= $1::timestamptz AND created_at < $2::timestamptz AND cache_usage_source = 'reported'), 0) AS total_reported_input_tokens,
			COALESCE(SUM(cache_creation_tokens)
				FILTER (WHERE created_at >= $1::timestamptz AND created_at < $2::timestamptz AND cache_usage_source = 'reported'), 0) AS total_reported_cache_creation_tokens,
			COALESCE(SUM(GREATEST(COALESCE(forced_cache_read_tokens, 0), 0))
				FILTER (WHERE created_at >= $1::timestamptz AND created_at < $2::timestamptz AND cache_usage_source = 'reported'), 0) AS total_reported_forced_cache_read_tokens,
			...
			COALESCE(SUM(input_tokens)
				FILTER (WHERE created_at >= $3::timestamptz AND created_at < $4::timestamptz AND cache_usage_source = 'reported'), 0) AS today_reported_input_tokens,
			COALESCE(SUM(cache_creation_tokens)
				FILTER (WHERE created_at >= $3::timestamptz AND created_at < $4::timestamptz AND cache_usage_source = 'reported'), 0) AS today_reported_cache_creation_tokens,
			COALESCE(SUM(GREATEST(COALESCE(forced_cache_read_tokens, 0), 0))
				FILTER (WHERE created_at >= $3::timestamptz AND created_at < $4::timestamptz AND cache_usage_source = 'reported'), 0) AS today_reported_forced_cache_read_tokens,
...
func (r *usageLogRepository) GetUserDashboardStats(ctx context.Context, userID int64) (*UserDashboardStats, error) {
	...
	totalStatsQuery := `
		SELECT
			...
			COALESCE(SUM(input_tokens) FILTER (WHERE cache_usage_source = 'reported'), 0) as total_reported_input_tokens,
			COALESCE(SUM(cache_creation_tokens) FILTER (WHERE cache_usage_source = 'reported'), 0) as total_reported_cache_creation_tokens,
			COALESCE(SUM(GREATEST(COALESCE(forced_cache_read_tokens, 0), 0)) FILTER (WHERE cache_usage_source = 'reported'), 0) as total_reported_forced_cache_read_tokens,
	...
	todayStatsQuery := `
		SELECT
			...
			COALESCE(SUM(input_tokens) FILTER (WHERE cache_usage_source = 'reported'), 0) as today_reported_input_tokens,
			COALESCE(SUM(cache_creation_tokens) FILTER (WHERE cache_usage_source = 'reported'), 0) as today_reported_cache_creation_tokens,
			COALESCE(SUM(GREATEST(COALESCE(forced_cache_read_tokens, 0), 0)) FILTER (WHERE cache_usage_source = 'reported'), 0) as today_reported_forced_cache_read_tokens,
