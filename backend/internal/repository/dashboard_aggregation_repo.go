	...
	query := `
		WITH hourly AS (
			SELECT
				...
				COALESCE(SUM(input_tokens) FILTER (WHERE cache_usage_source = 'reported'), 0) AS reported_input_tokens,
				COALESCE(SUM(cache_creation_tokens) FILTER (WHERE cache_usage_source = 'reported'), 0) AS reported_cache_creation_tokens,
				COALESCE(SUM(GREATEST(COALESCE(forced_cache_read_tokens, 0), 0)) FILTER (WHERE cache_usage_source = 'reported'), 0) AS reported_forced_cache_read_tokens,
		...
		SELECT
			...
			hourly.provider_cache_read_tokens,
			hourly.cache_hit_requests,
			hourly.forced_cache_read_tokens,
			hourly.reported_input_tokens,
			hourly.reported_cache_creation_tokens,
			hourly.reported_forced_cache_read_tokens,
			hourly.reported_requests,
			hourly.estimated_requests,
			hourly.unavailable_requests,
			hourly.total_cost,
			hourly.actual_cost,
			hourly.account_cost,
		...
		DO UPDATE SET
			...
			forced_cache_read_tokens = EXCLUDED.forced_cache_read_tokens,
			...
			reported_forced_cache_read_tokens = EXCLUDED.reported_forced_cache_read_tokens,
			reported_requests = EXCLUDED.reported_requests,
			estimated_requests = EXCLUDED.estimated_requests,
			unavailable_requests = EXCLUDED.unavailable_requests,
	...
	query := `
		WITH daily AS (
			SELECT
				...
				COALESCE(SUM(forced_cache_read_tokens), 0) AS forced_cache_read_tokens,
				COALESCE(SUM(reported_input_tokens), 0) AS reported_input_tokens,
				COALESCE(SUM(reported_cache_creation_tokens), 0) AS reported_cache_creation_tokens,
				COALESCE(SUM(reported_forced_cache_read_tokens), 0) AS reported_forced_cache_read_tokens,
				COALESCE(SUM(reported_requests), 0) AS reported_requests,
		...
		SELECT
			...
			daily.provider_cache_read_tokens,
			daily.cache_hit_requests,
			daily.forced_cache_read_tokens,
			daily.reported_input_tokens,
			daily.reported_cache_creation_tokens,
			daily.reported_forced_cache_read_tokens,
			daily.reported_requests,
			daily.estimated_requests,
			daily.unavailable_requests,
			daily.total_cost,
			daily.actual_cost,
			daily.account_cost,
