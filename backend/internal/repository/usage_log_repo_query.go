...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		cacheUsageSource          sql.NullString
		forcedCacheReadTokens     sql.NullInt64
	)
	...
	log := &service.UsageLog{
		...
		ForcedCacheReadTokens:     int(forcedCacheReadTokens.Int64),
	...
	if cacheUsageSource.Valid {
		value := service.CacheUsageSource(cacheUsageSource.String)
		log.CacheUsageSource = &value
	}
