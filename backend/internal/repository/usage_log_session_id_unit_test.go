...
// sessionIDArgIndex locates session_id from the tail of the arg slice.
//
// New columns are appended after created_at so the hardcoded $1..$N placeholder
// runs elsewhere in this file stay valid, which means session_id drifts further
...
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	...
	idx := sessionIDArgIndex(prepared.args)
	sessionArg := prepared.args[idx]
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	sessionArg := prepared.args[sessionIDArgIndex(prepared.args)]
	ns, ok := sessionArg.(sql.NullString)
	require.True(t, ok, "session_id arg should be a sql.NullString, got %T", sessionArg)
	...
	nsEmpty := preparedEmpty.args[sessionIDArgIndex(preparedEmpty.args)].(sql.NullString)
